...
static Group_AV *new_group_av(const Logger *log, Group_Chats *g_c, void (*audio_callback)(Messenger *, uint32_t,
                              uint32_t, const int16_t *, unsigned int, uint8_t, unsigned int, void *), void *userdata)
{
    if (!g_c) {
        return nullptr;
    }
 ...
 * return -1 on failure.
 */
static int groupchat_enable_av(const Logger *log, Group_Chats *g_c, int groupnumber, void (*audio_callback)(Messenger *,
